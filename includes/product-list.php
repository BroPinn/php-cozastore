    ...
    <div class="col-sm-6 col-md-4 col-lg-3 p-b-35 isotope-item">
        <div class="block2">
            <div class="block2-pic hov-img0">
                <?php
                        $imagePath = !empty($product['image_path']) ? './uploads/images/' . basename($product['image_path']) : './uploads/images/placeholder.jpg';

                        // To check the correct directory file name of the image
                        // echo '<pre>Image Path: ' . htmlspecialchars($imagePath) . '</pre>';
                        // echo '<pre>File Exists: ' . (file_exists($imagePath) ? 'Yes' : 'No') . '</pre>';
                        ?>
                <img src="<?= htmlspecialchars($imagePath) ?>" alt="<?= htmlspecialchars($product['productName']) ?>"
                    class="img-fluid">
